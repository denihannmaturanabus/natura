...
const PlanillaDetail: React.FC<PlanillaDetailProps> = ({ empresa, isReadOnly = false, planillaId, onBack }) => {
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-center gap-0.5 sm:gap-1">
              <input
                type="number"
                value={comision}
                onChange={(e) => !isReadOnly && updateCommission(e.target.value)}
                disabled={isReadOnly}
                className="w-10 sm:w-12 text-base sm:text-xl font-black text-pink-500 bg-transparent text-center focus:outline-none disabled:opacity-70"
              />
              ...
              <div className="flex items-start gap-2 sm:gap-4 mb-3">
                <div className="flex-1 min-w-0">
                  <input
                    ...
                    disabled={isReadOnly}
                    className="w-full font-bold text-gray-800 outline-none text-lg sm:text-xl bg-transparent mb-3 placeholder-gray-300 disabled:opacity-70"
                  />
                  ...
                  <div className="space-y-2 mb-3">
                    {p.productos.map((prod) => (
                      <div key={prod.id} className={`flex items-center gap-2 rounded-2xl p-2.5 border-2 transition-all ${prod.pagado ? 'bg-green-50 border-green-300' : 'bg-white border-gray-200'}`}>
                        ...
                        <button 
                          onClick={() => !isReadOnly && updateProducto(p.id, prod.id, { pagado: !prod.pagado })}
                          disabled={isReadOnly}
                          className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${prod.pagado ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-green-400'} disabled:opacity-70 disabled:cursor-not-allowed`}
                        >
                        ...
                        <input
                          ...
                          disabled={isReadOnly}
                          className={`flex-1 min-w-0 text-sm font-medium outline-none bg-transparent ${prod.pagado ? 'text-gray-500 line-through' : 'text-gray-700'} placeholder-gray-300 disabled:opacity-70`}
                        />
                        <div className="flex items-center gap-0.5 flex-shrink-0">
                          ...
                          <input
                            ...
                            disabled={isReadOnly}
                            className={`w-16 text-sm font-bold outline-none bg-transparent text-right ${prod.pagado ? 'text-green-600' : 'text-gray-800'} disabled:opacity-70`}
                          />
                        </div>
                        {!isReadOnly && (
                          <button
