...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const readOnly = sessionStorage.getItem('gm_readonly') === 'true';
  ...
  const handleLogin = (pass: string, readOnly: boolean = false) => {
    setIsAuthenticated(true);
    setIsReadOnly(readOnly);
    setView('empresaSelector');
    sessionStorage.setItem('gm_auth', 'true');
    sessionStorage.setItem('gm_readonly', readOnly.toString());
  };
  ...
  const handleLogout = () => {
    ...
    sessionStorage.removeItem('gm_readonly');
    sessionStorage.removeItem('gm_empresa');
  };
